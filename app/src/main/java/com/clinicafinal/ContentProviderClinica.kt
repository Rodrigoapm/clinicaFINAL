 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    override fun query(
        ...
        val cursor = when (getUriMatcher().match(uri)) {
            URI_ANIMAL -> TabelaBDAnimal(db).query(colunas, selection, argsSeleccao, null, null, sortOrder)
            URI_RACA -> TabelaBDRaca(db).query(colunas, selection, argsSeleccao, null, null, sortOrder)
            URI_ANIMAL_ESPECIFICO -> TabelaBDAnimal(db).query(colunas, "${BaseColumns._ID}=?", arrayOf("${id}"), null, null, null)
            URI_RACA_ESPECIFICA -> TabelaBDRaca(db).query(colunas, "${BaseColumns._ID}=?", arrayOf("${id}"), null, null, null)
            else -> null
        }
    ...
    override fun getType(uri: Uri): String? =
        when (getUriMatcher().match(uri)) {
            URI_ANIMAL -> "$MULTIPLOS_REGISTOS/${TabelaBDAnimal.NOME}"
            URI_RACA -> "$MULTIPLOS_REGISTOS/${TabelaBDRaca.NOME}"
            URI_ANIMAL_ESPECIFICO -> "$UNICO_REGISTO/${TabelaBDAnimal.NOME}"
            URI_RACA_ESPECIFICA -> "$UNICO_REGISTO/${TabelaBDRaca.NOME}"
            else -> null
        }
    /**
 ...
 * and Threads](
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        ...
        val id = when (getUriMatcher().match(uri)) {
            ...
            URI_RACA -> TabelaBDRaca(db).insert(values)
            else -> -1
        }
 ...
 * and Threads](
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<out String>?): Int {
        ...
        val registosApagados = when (getUriMatcher().match(uri)) {
            URI_ANIMAL_ESPECIFICO -> TabelaBDAnimal(db).delete(
                "${BaseColumns._ID}=?",
                arrayOf("${id}")
            )
            URI_RACA_ESPECIFICA -> TabelaBDRaca(db).delete(
                "${BaseColumns._ID}=?",
                arrayOf("${id}")
            )
            else -> 0
 ...
 * and Threads](
    ...
    override fun update(
        ...
        val registosAlterados = when (getUriMatcher().match(uri)) {
            ...
            URI_RACA_ESPECIFICA -> TabelaBDRaca(db).update(values,"${BaseColumns._ID}=?", arrayOf("${id}"))
            else -> 0
        }
    ...
    companion object {
       private const val AUTHORITY = "com.clinicafinal"
       private const val URI_RACA = 100
       private const val URI_RACA_ESPECIFICA = 101
       private const val URI_ANIMAL = 200
       private const val URI_ANIMAL_ESPECIFICO = 201
       ...
       private const val MULTIPLOS_REGISTOS = "vnd.android.cursor.dir"
        ...
        val ENDERECO_ANIMAL = Uri.withAppendedPath(ENDERECO_BASE, TabelaBDAnimal.NOME)
        val ENDERECO_RACA = Uri.withAppendedPath(ENDERECO_BASE, TabelaBDRaca.NOME)

        fun getUriMatcher() : UriMatcher {
            var uriMatcher = UriMatcher(UriMatcher.NO_MATCH)

            uriMatcher.addURI(AUTHORITY, TabelaBDRaca.NOME, URI_RACA)
            uriMatcher.addURI(AUTHORITY, "${TabelaBDRaca.NOME}/#", URI_RACA_ESPECIFICA)
            uriMatcher.addURI(AUTHORITY, TabelaBDAnimal.NOME, URI_ANIMAL)
            uriMatcher.addURI(AUTHORITY, "${TabelaBDAnimal.NOME}/#", URI_ANIMAL_ESPECIFICO)
